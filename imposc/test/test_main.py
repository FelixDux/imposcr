...
@pytest.mark.parametrize(('path', 'status', 'response_json'), [
    ...
    ("garbage", 404, {"detail": "Not Found"})
])
def test_read_parameter_info(path, status, response_json):
    response = client.get(f"/api/parameter-info/{path}")
