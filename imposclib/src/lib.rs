...
impl ParameterProperties {
    fn new() -> ParameterProperties
 {
    ...
    fn populate(&mut self, records: Vec<(String, String)>) {
        self.properties.clear();

        records.into_iter().for_each(|(parameter, property)| {self.properties.entry(parameter).or_insert(property);});
    }
...
mod tests {
    ...
    fn can_access_parameter_properties() {
        ...
        properties.populate(vec![(String::from("frequency"), String::from("ω"))]);
